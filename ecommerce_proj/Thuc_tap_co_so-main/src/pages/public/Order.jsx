...
export default function Order({ onClose, subtotal, shippingFee, selectedItems }) { 
    ...
    // Tính toán tiền
    const shopDiscountTotal = Object.entries(selectedShopVouchers).reduce((total, [shopName, voucherId]) => {
        if (!voucherId) return total; // Nếu không chọn voucher thì bỏ qua

        // Dò tìm voucher trong danh sách để lấy ra số %
        const voucher = vouchers.shops[shopName]?.find(v => v.id.toString() === voucherId.toString());
        const discountPercent = voucher ? voucher.value : 0;

        // Tính tổng tiền của shop đó
        const shopSubtotal = cartItems
            .filter(item => item.shop === shopName)
            .reduce((sum, item) => sum + (item.price * item.quantity), 0);

        // Nhân %
        const discountAmount = shopSubtotal * (discountPercent / 100);
    ...
    const handlePlaceOrder = async () => {
        ...
        // 1. BẠN PHẢI THÊM 2 DÒNG NÀY ĐỂ TẠO RA BIẾN shopVoucherId TRƯỚC:
        const selectedVoucherIds = Object.values(selectedShopVouchers).filter(id => id !== "");
        const shopVoucherId = selectedVoucherIds.length > 0 ? Number(selectedVoucherIds[0]) : null;

        // 2. SAU ĐÓ MỚI CÓ THỂ NHÉT NÓ VÀO ĐÂY ĐỂ GỬI ĐI:
        const orderRequest = {
            userId: userId,
            ...
            shopVoucherId: shopVoucherId // Lúc này biến đã được tạo ở trên nên không bị lỗi nữa
        };
                        ...
                        <div className="voucher-selection-area">
                            ...
                            {uniqueShops.map(shopName => (
                                <div className="voucher-group" key={shopName}>
                                    ...
                                    <select onChange={(e) => {
                                        const vId = e.target.value; // Lấy ID của voucher
                                        setSelectedShopVouchers(prev => ({...prev, [shopName]: vId}));
                                    }}>
                                        <option value="">Chọn mã giảm giá shop...</option>
                                        {vouchers.shops[shopName]?.map(v => (
                                            <option key={v.id} value={v.id}>  {/* ĐỔI value={v.value} THÀNH value={v.id} */}
                                                {`Giảm ${v.value}%`}
                                            </option>
                                        ))}
                                    </select>
