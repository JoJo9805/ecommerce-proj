...
export default function Account() {
    ...
    const submitReview = async () => {
        ...
        try {
            ...
            if (response.ok) {
                // 1. ĐỌC DỮ LIỆU TỪ BACKEND API TRẢ VỀ (Cái API mà bạn bị thiếu)
                const resultMessage = await response.text(); 
                
                // Hiển thị thông báo (Thành công hoặc Bị cảnh báo Spam)
                alert(resultMessage); 

                // 2. KIỂM TRA XEM CÓ PHẢI SPAM KHÔNG ĐỂ CẬP NHẬT GIAO DIỆN
                const isSpam = resultMessage.toLowerCase().includes("spam");

                const today = new Date().toLocaleDateString("vi-VN");
                const updatedHistory = orderedHistory.map(item => 
                    (item.key === reviewTarget.key) ? { 
                        ...item, 
                        // Nếu Spam thì hiện chữ "Đang chờ duyệt", không Spam thì hiện bình luận thật
                        review: isSpam ? "Đang chờ kiểm duyệt (Nghi ngờ Spam)" : reviewForm.comment, 
