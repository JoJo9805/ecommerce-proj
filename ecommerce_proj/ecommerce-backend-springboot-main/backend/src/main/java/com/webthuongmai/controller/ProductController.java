...
public class ProductController {
    ...
    @GetMapping("/trending")
    public ResponseEntity<List<ProductCardDTO>> getTrending() {
        return ResponseEntity.ok(productService.getTrendingProducts(10));
    }

    // Hiển thị theo lịch sử tìm kiếm (Gợi ý cho User)
    @GetMapping("/recommendations")
    public ResponseEntity<List<ProductCardDTO>> getRecommendations(@RequestParam(required = false) Long userId) {
